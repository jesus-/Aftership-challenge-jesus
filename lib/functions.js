...
*/
  sortJsonByDate: function(objJson){
    return objJson.sort(function (a,b){
      if(a.checkpoint_time == b.checkpoint_time){
        return 0;
      }else if(a.checkpoint_time < b.checkpoint_time){
        return 1;
      }else if(a.checkpoint_time > b.checkpoint_time){
        return -1;
      }
...
/*Receive a string
  Return the same string whiout double spaces
*/
deleteDoubleSpaces: function(string_change){
  while(string_change.indexOf('  ')!==-1){
    string_change = string_change.replace('  ',' ');
  }
